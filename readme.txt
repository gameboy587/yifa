...
2. 填充腾讯云的那个数据库
3. 测试django搭建的web service, 先看看能不能正确获取orderId, 然后要去数据库找用户
account, 这部分我之前qq云太卡一直登陆不上看不到信息,,你们谁看一下..
然后找到了直接调用api那个就可以推送了

你们可以看看python的代码, 大部分都是自动生成的,主要只有一个文件:
web_service/views.py,
https://github.com/gameboy587/yifa/blob/master/web_service/views.py
python真的很简单的, 代码估计加起来40行吧...
